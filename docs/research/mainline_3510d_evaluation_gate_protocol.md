...
## 11. 4.3 复权/停牌/涨跌停审计（2026-08-14）

**发现 2 个 P1 缺陷（均已修复）**：

1. **行间停牌标签污染（multi_horizon.py）**：TuShare 停牌日不产生行，`_window_has_issue` 只能标记 volume==0 行，无法感知行间缺失——停牌前的 label 用复牌后价格计算（跨停牌跳变收益）。实证：601088（停牌 18 天）2025-08-01 label_5d=0.0250 实际是 8-01→8-21 跨停牌收益。
   → 修复：`_suspension_gap_mask`（断档 >15 自然日=停牌，与 2.2 阈值一致）；停牌股各 ~30 个 NaN 标签；+2 单元测试。
2. **qfq 增量边缘整段重取（tushare_source.py）**：请求 start=20230101 而缓存从 2023-01-03 起（元旦周末无行）→ `_date_ranges_to_fetch` 算出增量 (01-01, 01-02) → qfq 分支整段重取 865+ 天（每次读取都触发！）。实证：fetch 调用 (20230101, 20260813) 全量。
   → 修复：增量区间完全位于 existing 之前（前端边缘，无复权接缝）时直接增量拼接；尾部/中间洞仍整段重取。

**涨跌停**：t 涨停日 close_to_close 标签从涨停价起算（实盘无法按涨停价买入）→ 与 4.1 的 close_to_close 口径差距合并，由 next_open_to_open 升级解决。

**后续动作（决策记录）**：标签逻辑变更已修复，但**当前数据集（sequence_baseline_20230101_20260813）含跨停牌污染标签**——5.x 同窗比较前必须重建数据集并重跑基线，否则基线比较带污染。
