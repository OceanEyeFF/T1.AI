...
This package implements the phase-1 MVP sequence dataset builder:
convert feature/label DataFrames into fixed-length time series samples:
`X` with shape ``[N, seq_len, n_feat]`` and `y` with shape ``[N, n_label]``.

Key design:
  - Sliding window: fixed ``seq_len`` and step ``stride`` to generate samples.
  - Time alignment: sample with label timestamp ``t`` only uses features from
